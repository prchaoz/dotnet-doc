...
description: Inspect .NET type signatures and documentation before implementing interfaces or overriding classes. Use dotnet-doc CLI to look up type members, XML doc comments, and referenced types.
...
Before implementing an interface or overriding a class, run `dotnet-doc` to inspect the type's full API surface.
...
```bash
dotnet-doc --runtime netcore/10.0 --all System.IO.Stream
```
...
3. Run `dotnet-doc` to see all members, their signatures, and doc comments
